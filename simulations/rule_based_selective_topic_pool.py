...
def encode_demographics(state):
    """
    Convert categorical demographic features into a one-hot encoded numpy array.
    """
...
def effective_engagement(state):
    """
    Create a feature vector from the user state.
    The vector consists of the engagement score, demographics, and latent taste.
    """
...
def compute_policy(prior, state, gamma):
    ...
    features = effective_engagement(state)
...
def define_topic_pools(topic_engagement_dict, threshold=0.3):
...
topics_pool = define_topic_pools(topic_engagement_dict, threshold=0.5)

# ---------------------------
# Simulation Function with Engagement-Dependent Topic Pools
# ---------------------------
def simulation_exp_cond_bin(N, T, tau0, lambda_, k_min, k_max, version, topics_pool, 
